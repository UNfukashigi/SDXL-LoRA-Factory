...
・依存関係の不足やバージョン差によるエラー対策として、voluptuous を requirements に追加し、transformers を4系（transformers>=4.44,<5）に固定しました。これにより、新規環境でのセットアップ失敗や transformers 5系との互換性問題を避けやすくしています。- Added voluptuous to requirements and constrained transformers to version 4.x (transformers>=4.44,<5) to reduce setup failures and compatibility issues with transformers 5.x in fresh environments.<br>
